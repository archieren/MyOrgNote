...
......
~"我们现在超越网格细胞的计算起源,转向理解这些细胞在训练神经网络中的机制起源."~ -- ~“设定什么约束条件,必然得到这种网格编码?”~
...
但要关注的是[Schaefer R.;...;Fiete I.R.]2023的工作,他们认为这些工作,设定的前提是有问题的, ~为什么要以位置细胞的数据作为监督目标?~
...
***** Fiete I.R.毕竟是这个领域的大牛,一上来就批判性讨论,
- “为什么”和“怎么做”的问题: 传统模型长期以来难以解释网格细胞存在的“目的”或“根本原因”(即目的论原因，Teleological cause)
- 进一步剥离了人为的监督信号假设,证明了网格细胞并非为了“拟合位置”而存在，
- 而是作为一种高容量、高鲁棒性的编码方案，在大脑进行自我组织和路径积分的过程中自然涌现。
作者提出了一种自监督学习框架，完全抛弃了位置标签，仅依靠速度输入和神经网络内部的约束来生成网格细胞。
***** 方法论：SIC框架
作者从编码理论、动力系统和优化理论中提取了四个核心原则，构建了名为 SIC 的损失函数组合：
- Separation (分离原则)：要求神经网络中代表不同空间位置的状态在向量空间中要尽可能分得开。这保证了编码的区分度。
- Invariance (不变性原则)：要求网络对“路径”具有不变性。即，无论动物走哪条路，只要终点相同，最终的神经状态应该是一样的。这是路径积分和连续吸引子动力学的核心特征。
- Capacity (容量原则)：这是该论文的一大亮点。作者认为大脑倾向于用有限的神经元编码尽可能多的位置。在数学上，这通过让神经表示倾向于聚集在一个紧凑的流形上来实现。
- Conformal Isometry (保角等距原则)：要求物理空间的小距离移动，对应神经空间的小距离变化，且变化大小与方向无关，只与距离有关。
***** 基于Conformal Isometry Hypothesis的工作！
我没想到,Wu Ying Nian也在搞这一块的工作!
[2025 - On Conformal Isometry of Grid Cells - Learning Distance-Preserving Position Embedding]
***** 奇文: [2025 - Spatial periodicity in grid cell firing is explained by a neural sequence code of 2-D trajectories]
我不知该不该深入理解他一下,故称之为奇文!
...
目前来看,这类工作都被归到Latent Space Reasoning这个主题之下!
...
** Google又来一个三部曲！Titans -> Miras -> Nested Learning
Sequence Modeling 和 Association Memory联系了起来!
Titans是架构,Miras是理论蓝图! Nested Learning揭示和神经科学的关系!
* ※※※※※※※※
...
[[file:images/ssm-hippo-s4-mamba.jpg][SSM-To-Mamba]]
 - [Gu A.;...]2020 - HiPPO - Recurrent Memory with Optimal Polynomial Projections
   ...
   - 通过“正交函数基”对“持续更新的函数”进行实时逼近，-- “这是有点聪明的地方:正交函数基的定义域是固定的，而持续更新的函数的定义域是持续更新的!”
   - 其拟合的投影系数的动力学正好可以表示为一个线性ODE系统， -- ~这延伸一个问题,投影系数的动力系统,真的能解决“记忆问题”?~
   - 并且对于特定的基底以及逼近方式，我们可以将线性系统的关键矩阵精确地算出来. -- 这个是要点功力的!
   HiPPO是个关键,有一定的理论意义!!! 或者说,强行给“状态变迁矩阵”的设计添加一个说得过去的理由,这最终导致了后继工作中,“结构化”的启发!
   这篇文章的表达,实在不敢恭维!
 ...
 - [Gu A.;...]2022 - S4 - Efficiently Modeling Long Sequences with Structured State Spaces
   这篇文章的贡献:
   - 主要集中在计算方法的优化!!!“结构化”的矩阵易于优化,即所谓的HIPPO矩阵等等才是此类!!!
   - “结构化的涵义”

   S4(Structured State Space sequence),所谓的结构化方法
   ...
   - 事实上,S4之后,已经不讨论“结构化”矩阵问题,而是直接使用对角矩阵了！
   ...
   - "We argue that a fundamental problem of sequence modeling is compressing context into a smaller state!!"
     ...
     - 解决问题的方法之一,就是"Selection". 但,什么是"Selection"?
       - 但Selection,具体地说, selective scan,又破坏了S4中的卷积递归双面性!!!
         ...
         不过此时,有并行算法存在!(Parallel scan(Parallel prefix sum))
...
+ Linear RNN ~~> Linear Attention 等等,这里可能有些简单的分类问题,可以不必纠结?
  ...,Linear Transformer,RetNet,RWKV,DeltaNet, [TTT,Mamba2,Gated Deltanet,], Titans...,可以去看看,科学空间的几篇文章,
...
+ [[ΔΔ]2024 - DeltaNet - Parallelizing Linear Transformers with the Delta Rule over Sequence Length]
  [[ΔΔ]2024 - Gated Delta Networks - Improving Mamba2 with Delta Rule - v3]
  -- QWen3-Next的混合注意力路线中就采用了Gated Delta Networks!
