...
[[file:images/ssm-hippo-s4-mamba.jpg][SSM-To-Mamba]]
 ...
 - [Gu A.;...]2021 - LSSL - Combining Recurrent, Convolutional, and Continuous-time Models with Linear State-Space Layers
   之前: "Continuous State Space Model" -> "Discrete-time State Space Models: The recurrent representation"
   而这个LSSL: => "Training SSMs: The Convolutional Representation"
   这是一篇过渡文章,将SSM的两个视角(As a recurrence Or As a convolution)作了统一!
 ...
 - [Gu A.;...]2022 - S4 - Efficiently Modeling Long Sequences with Structured State Spaces
   ...
   - the convolutional representation: 可高效计算Kyrov Function(即那个SSK函数)的参数矩阵是对角化矩阵.
     矩阵的共轭等价变化,可得到对角矩阵.
     论文"2022 - DSS - Diagonal State Spaces are as Effective as Structured State Spaces"首先发现了这点!
 ...
 - [CMU]2023 - Mamba - Linear-Time Sequence Modeling with Selective State Spaces
   - "We argue that a fundamental problem of sequence modeling is compressing context into a smaller state!!"
     ...
     - S4的变种或衍生.
       略.讨论了什么?
     - 基于SSM的架构: To refer to deep neural network architectures incorporating one of the previous SSMs as a black box layer.
       Linear Attention, GSS , Mega ,H3,Selective S4,RetNet,RWKV等等...?? 间接或直接都和SSM挂上了勾?
