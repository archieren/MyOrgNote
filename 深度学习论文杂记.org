...
MaskFormer --> Mask2Former --> OneFormer
(MaX-DeepLab 和 MaxFormer的对比,很有意思,在争论谁先落实 pixel classification 到 mask classification的转变)
SegFormer (For semantic segmentation)--> Panoptic SegFormer (For panoptic segmentation)
DiNo --> Mask DiNo (Can Detection and Segmentation be unified)
