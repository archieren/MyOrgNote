...
* Memory怎么和Agent Evolve挂钩了？
~Static -> Dynamic -> Evolutive~ :Memory 机制的N步曲!
- Self-Evolving Memory
  ...
  ...
- (Structured) Dynamic Memory
  [Dynamic Cheatsheet: Test-Time Learning with Adaptive Memory]
  [Zep: A Temporal Knowledge Graph Architecture for Agent Memory]
  ...
** 纵观上面一系列文章,“Memory Mechanism”承担一个关键作用!演化在于记忆的更迭!
*** 长文[2025 - Memory in the Age of AI Agents - A Survey]
这篇文章对目前Memory相关研究的分类,更具清晰性和新颖性!
**** 研究的视角: 通过形式、功能和动态(Forms, functions, and dynamics)的视角，提供当前研究的统一和全面的全景。
  - Forms: 三种记忆实现(令牌级、参数级、潜在记忆)
  - Functions: 三种记忆功能(事实记忆、经验记忆、工作记忆),这和以往的长短期记忆划分方式有明显的差异！
  - Dynamics: 记忆如何在代理与环境互动的过程中形成、演变和检索.
**** 作为2025年末出的总结,确实是一片好文,将这几年围绕Memory作了个非常好的总结?
*** DeepSeek的ENGRAM,又搞了个Conditional Memory,该放到何处?似乎又引发了Memory 和Latent Reasoning的关联互动!
...
？问题:那些是适合知识图谱的行业？
** Agentic Rag代表了什么进展？
- [2025 - Agentic Retrieval-Augmented Generation - A Survey on Agentic RAG - v1]
  总结了Rag的架构模式!: Naïve RAG |> Advance Rag |> Modular Rag |> Graph Rag |> Agentic Rag
  Agentic Rag 是建立在 Modular Rag 上的, 但克服了 Moduar Rag的一些内在问题!
- [2025 - Towards Agentic RAG with Deep Reasoning - A Survey of RAG-Reasoning Systems in LLMs - v2.pdf]
  从RAG、Reasoning两者互增强到协同的角度进行了总结!
RAG -> GraphRAG,KG?RAG,HiRAG,LeanRAG,... -> Agentic RAG(如 Search-o1等等)
*** ~Graphic Rag 和 Agentic Rag事实上,按我的理解,应当是关于知识、记忆的两个维度: Structure(Realation, Logical, Ontology...) and Dynamic(Context, Adaption, ?Learning, Evolution,)!~
*** 还有个话题,RAG和Memory的关系! - ~RAG目前大部分是放在Memory机制范畴下的~
*** 但首先还是先了解Agentic Rag的基础 Agentic Intelligence!
...
AI Agent在架构分类和实际部署中具有三大基础特征: 自主性(Autonomy)、任务专一性(Task Specification)以及反应性与适应性(Reactivity and Adaptivity).
Agent的架构范式, 从抽象的 ~Agent = Perception + Reasoning + Action~ 到目前流行的OpenAI给出的较具体的 ~Agent = LLM + Planning + Memory + Tool Use~.
Agentic AI是一种新兴的智能架构，利用(多个)专业化AI Agent(协作)驱动某类过程，实现复杂且高层次的目标。
例如: ~用 Agent 的思维去驱动 RAG 过程，让检索变得更聪明，这就是所谓的Agentic RAG。~
***** 那么在Agentic AI下,就有很多内容要讨论!
- 主题:Agentic RAG, Agentic Memory, Agentic RL,Agentic Deep Research等等
...
** LLM之RAG最新进展 - 检索从相似匹配变成动态学习?
- [2025 - CLaRa - Bridging Retrieval and Generation with Continuous Latent Reasoning]
  这条道路??? Latent Space到底可以承担多少大任?
